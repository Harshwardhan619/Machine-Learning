...
def read_image(filename):
	...
	dimesions = f.readline().split()
	dimesions = [int(p) for p in dimesions]
	max_val = int(f.readline())
	img = []
	for p in range(dimesions[1]):
		temp_line = []
		for q in range(dimesions[0]):
			if(format_name	 == "P6\n"):
				temp = [f.read(1), f.read(1), f.read(1)]
				temp = [int("{:02x}".format(ord(z)), 16) for z in temp]
				# reference = https://stackoverflow.com/a/35516257/8441259
				temp_line.append(temp)
			elif(format_name == "P5\n"):
				temp = f.read(1)
				temp = int("{:02x}".format(ord(temp)), 16)
				temp_line.append(temp)
		img.append(temp_line)
...
def write_image(filename, img):
	...
	to_write += str(len(img[0])) + " " + str(len(img)) + "\n"
...
def decompress_image(cluster_labels, cluster_centroids):
	...
	img = cluster_labels
	for p in range(len(cluster_labels)):
		for q in range(len(cluster_labels[p])):
			z = list(cluster_centroids[cluster_labels[p][q]])
			z = [int(r) for r in z]
			# print (z)
			img[p][q] = z
